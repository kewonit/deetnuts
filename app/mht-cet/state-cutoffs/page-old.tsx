...
export default function StateCutoffsPage() {
  ...
  useEffect(() => {
    const filtersChanged =
      ...
      JSON.stringify(pendingFilters.categories) !==
        JSON.stringify(filters.categories) ||
      JSON.stringify(pendingFilters.courses) !==
        JSON.stringify(filters.courses) ||
      JSON.stringify(pendingFilters.statuses) !==
        JSON.stringify(filters.statuses) ||
      JSON.stringify(pendingFilters.homeUniversities) !==
        JSON.stringify(filters.homeUniversities) ||
  ...
  const fetchRecords = useCallback(async () => {
    if (
      !debouncedFilters.percentileInput ||
      debouncedFilters.percentileInput.trim() === ""
    ) {
      setRecords([]);
      setTotalItems(0);
      setLoading(false);
    ...
    try {
      ...
      if (!result.success) {
        ...
        if (result.error === "Data not available") {
          toast.error(
            `${getDisplayNameForRound(debouncedFilters.round)} data is not available yet. Please try Round 1 instead.`,
          );
  ...
  const handlePageChange = useCallback(
    (newPage: number) => {
      ...
      if (
        newPage >= 1 &&
        newPage <= Math.ceil(memoizedTotalItems / itemsPerPage)
      ) {
      ...
      <div className="pt-20 lg:pt-6">
        <div className="flex flex-col gap-3">
          <h1 className="text-3xl lg:text-3xl xl:text-4xl font-black tracking-tight text-gray-900 font-inter break-words">
            MHT-CET State Cutoffs {filters.year} -{" "}
            {getDisplayNameForRound(filters.round)}
          </h1>
          <p className="text-gray-600 text-sm md:text-base lg:text-lg font-medium font-inter">
            Use these to predict your chances of admission based on your
            percentile. (Data : {filters.year}{" "}
            {getDisplayNameForRound(filters.round)})
          </p>
        </div>
      </div>
