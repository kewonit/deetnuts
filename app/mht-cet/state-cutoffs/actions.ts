...
export async function getCutoffRecords(
    ...
    try {
        ...
        const buildFilterParts = (
            ...
            if (homeUniversitiesToFilter && Array.isArray(homeUniversitiesToFilter) && homeUniversitiesToFilter.length > 0) {
            ...
            if (statusesToFilter && Array.isArray(statusesToFilter) && statusesToFilter.length > 0) {
        ...
        if (shouldSplitQuery) {
            ...
            console.log(`Splitting query into chunks - Categories: ${categoryChunks.length}, Courses: ${courseChunks.length}, Statuses: ${statusChunks.length}, Universities: ${homeUniversityChunks.length}`);
