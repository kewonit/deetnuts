...
export function useCutoffData(): UseCutoffDataReturn {
    ...
    const cacheRef = useRef<Map<string, { data: CutoffRecord[]; totalItems: number; timestamp: number }>>(new Map());
    ...
    const fetchData = useCallback(async (params: FetchParams) => {
        ...
        if (cacheRef.current.has(cacheKey)) {
            ...
            if (JSON.stringify(cached.data) !== JSON.stringify(lastRecordsRef.current) || 
        ...
        try {
            ...
            if (!result.success) {
                if (result.error === 'Data not available') {
                    toast.error(`${getDisplayNameForRound(params.round)} data is not available yet.`);
                } else {
