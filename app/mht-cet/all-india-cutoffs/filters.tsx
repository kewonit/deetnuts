...
import { Search, X, Filter, RotateCcw, Loader2, Check, ChevronDown, GraduationCap } from 'lucide-react';
...
export function Filters({ filters, onFiltersChange, onSearch, loading }: FiltersProps) {
    ...
    const updateBranches = useCallback((newBranches: string[]) => {
        onFiltersChange({
            ...filters,
    ...
    const toggleGroup = useCallback((groupName: string) => {
        const groupBranches = COURSE_GROUPS[groupName as keyof typeof COURSE_GROUPS] || [];
        ...
        if (allGroupSelected) {
            ...
            updateBranches(currentBranches.filter(branch =>
                !availableGroupBranches.includes(branch)
            ));
        } else {
            // Add all group branches (avoid duplicates)
            const newBranches = [...new Set([...currentBranches, ...availableGroupBranches])];
            ...
            <CardHeader className="pb-3 bg-[#E4DFF2] border-b-2 border-black">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0">
                    <div className="flex items-center space-x-2">
                        <CardTitle className="text-lg font-bold text-black">Filters</CardTitle>
                        {activeFilterCount > 0 && (
                            <Badge variant="neutral" className="text-xs border-2 border-black shadow-base bg-white">
            ...
            <CardContent className="space-y-4 p-4 sm:p-6">
                ...
                <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
                    <div className="flex-1">
                        <Label htmlFor="search" className="text-sm font-medium text-black">Search</Label>
                ...
                <div>
                    <Label htmlFor="branch" className="text-sm font-medium text-black flex items-center space-x-2 mb-2">
                    ...
                    <Popover open={branchPopoverOpen} onOpenChange={setBranchPopoverOpen}>
                                ...
                                <div className="flex flex-wrap gap-1.5 max-w-[calc(100%-40px)]">
                                    {filters.branches && filters.branches.length > 0 ? (
                                        ...
                                        <div className="flex items-center gap-2 text-gray-600">
                                            <GraduationCap className="h-4 w-4" />
                                            <span>Select engineering branches or entire groups...</span>
                                        </div>
                                    )}
                                </div>
                        ...
                        <PopoverContent className="w-full min-w-[350px] sm:min-w-[500px] max-w-[95vw] sm:max-w-[700px] p-0 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] rounded-base bg-white" align="start">
                            ...
                            <Command className="border-none">
                                ...
                                <CommandList className="max-h-96 overflow-y-auto">
                                    ...
                                    {Object.entries(COURSE_GROUPS).map(([groupName, branches]) => {
                                        const availableGroupBranches = branches.filter(branch =>
                                            ENGINEERING_BRANCHES.includes(branch as any)
                                        );
                                        const currentBranches = filters.branches || [];
                                        const selectedInGroup = availableGroupBranches.filter(branch =>
                                            currentBranches.includes(branch)
                                        ).length;
                                        const allGroupSelected = selectedInGroup === availableGroupBranches.length;
                                                        ...
                                                        <div className="flex items-center justify-between w-full">
                                                            <div className="flex items-center space-x-3">
                                                                <Checkbox
                                                                    ...
                                                                    className={`border-2 border-black h-5 w-5 ${someGroupSelected && !allGroupSelected
                                                    ...
                                                    {availableGroupBranches.map((branch, index) => {
                                                        ...
                                                        return (
                                                            <CommandItem
                                                                ...
                                                                className={`pl-8 py-2.5 hover:bg-blue-50 ${index === availableGroupBranches.length - 1
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            {(filters.minPercentile || filters.maxPercentile) && (
                                <Badge variant="neutral" className="text-xs">
                                    Percentile: {filters.minPercentile || '0'} - {filters.maxPercentile || '100'}
                                    <Button
                                        variant="noShadow"
                                        size="sm"
                                        className="h-4 w-4 p-0 ml-1"
