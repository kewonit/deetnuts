...
export async function GET(request: Request, props: { params: Promise<{ id: string }> }) {
    ...
    try {
        ...
        const records = await pb.collection('2024_mht_cet_round_one_cutoffs_duplicate').getFullList({
        ...
        return new NextResponse(
            JSON.stringify({ message: 'Failed to fetch cutoff data', error: error.message }),
