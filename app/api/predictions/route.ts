...
export async function GET(request: NextRequest) {
    try {
        ...
        // Range filters
        const minCutoff = searchParams.get('minCutoff') ? parseFloat(searchParams.get('minCutoff')!) : null;
        const maxCutoff = searchParams.get('maxCutoff') ? parseFloat(searchParams.get('maxCutoff')!) : null;
        const minChange = searchParams.get('minChange') ? parseFloat(searchParams.get('minChange')!) : null;
        const maxChange = searchParams.get('maxChange') ? parseFloat(searchParams.get('maxChange')!) : null;
        
        // Read CSV file
        const filePath = path.join(process.cwd(), 'scripts', 'predictions_2026_complete.csv');
        ...
        const records: PredictionRecord[] = parse(fileContent, {
            ...
            cast: (value, context) => {
                ...
                if (context.column === 'predicted_cutoff_2026' || 
        ...
        let filteredRecords = records.filter(record => {
            ...
            if (!record.predicted_cutoff_2026 || record.predicted_cutoff_2026 <= 0 || isNaN(record.predicted_cutoff_2026)) {
                return false;
            }
            ...
            // Range filters
            if (minCutoff !== null && record.predicted_cutoff_2026 < minCutoff) return false;
            if (maxCutoff !== null && record.predicted_cutoff_2026 > maxCutoff) return false;
            if (minChange !== null && record.change_from_2025 < minChange) return false;
            if (maxChange !== null && record.change_from_2025 > maxChange) return false;
        ...
        filteredRecords.sort((a, b) => {
            ...
            if (typeof aValue === 'number' && typeof bValue === 'number') {
                ...
                if (sortBy === 'change_from_2025' || sortBy === 'pct_change_from_2025') {
        ...
        const uniqueInstitutes = [...new Set(records.map(r => r.institute))].sort();
        const uniqueBranches = [...new Set(records.map(r => r.branch))].sort();
        const uniqueSeatTypes = [...new Set(records.map(r => r.seat_type))].sort();
        ...
        const stats = {
            ...
            averagePredictedCutoff: filteredRecords.reduce((sum, r) => sum + r.predicted_cutoff_2026, 0) / filteredRecords.length || 0,
            averageChange: filteredRecords.reduce((sum, r) => sum + r.change_from_2025, 0) / filteredRecords.length || 0,
            averagePercentChange: filteredRecords.reduce((sum, r) => sum + r.pct_change_from_2025, 0) / filteredRecords.length || 0,
        };
