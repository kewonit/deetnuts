...
export default async function FieldPage(props: any) {
  ...
  const fieldTitle = decodedField
    ...
    .map((word: string) => word.charAt(0).toUpperCase() + word.slice(1))
    .join(" ");
  const colorIndex = fields.findIndex(
    (f) => f.toLowerCase() === fieldTitle.toLowerCase(),
  );
  ...
  return (
    <div className={montserrat.className}>
      <main className="min-h-screen bg-gradient-to-r from-violet-200 to-pink-200 p-4 sm:p-8 mt-16 overflow-hidden">
        ...
        <div
          className={`${bgColor} border-8 border-black shadow-[16px_16px_0px_0px_rgba(0,0,0,1)] p-8 mb-8`}
        >
          <h1 className="text-4xl sm:text-7xl font-black text-white mb-4 leading-none">
            {fieldTitle.split(" ").map((word: string, index: number) => (
              <span
                key={index}
                className="inline-block transform -skew-y-3 mr-2"
              >
                {word}
              </span>
            ))}
        ...
        <div className="mb-12 relative mt-24 p-4">
          ...
          <div className="absolute -top-6 -right-6 w-20 h-20 bg-yellow-300 rounded-full border-4 border-black z-10 flex items-center justify-center shadow-lg">
            <span className="font-black text-xl text-black transform -rotate-12">
              NEW
            </span>
          </div>
        </div>
        ...
        <Card className="border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
          <CardContent className="p-6">
            <h2 className="text-2xl font-bold mb-4">
              Top Institutions in {fieldTitle}
            </h2>
            <ScrollArea className="h-[60vh]">
              <ul className="space-y-2">
                ...
                {[...Array(20)].map((_, index) => (
                  <li
                    key={index}
                    className="bg-gray-100 p-4 rounded-lg border-2 border-black"
                  >
