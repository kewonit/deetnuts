    ...
    >
        ...
        <div className="policy-table-wrap" role="region" aria-label="Service providers table" tabIndex={0}>
          <table className="policy-table">
            ...
            <tbody>
              ...
              <tr><th scope="row">Cloudflare</th><td data-label="Function">Authoritative DNS, TLS termination, content delivery, request routing, caching, abuse protection and identity-gated administrator access.</td><td data-label="Information involved">IP address, requested hostname and route, HTTP and TLS metadata, device or browser information, approximate location, security signals, performance and error information and, for the restricted administrator route, the Google-verified email address and authentication result.</td><td data-label="When">When the Service is requested through its public domains or an authorised administrator opens the protected API hostname.</td></tr>
      ...
      <section id="retention">
        ...
        <ul>
          ...
          <li><strong>Backups:</strong> PocketBase creates a daily local application backup and retains up to seven ordinary copies. Before a high-risk infrastructure change, a fresh verified archive may also be encrypted and held off the VM, and a powered-off DigitalOcean snapshot may be kept for a short rollback window. The VM is not enrolled in recurring DigitalOcean managed backups. A restored backup follows the same purpose and retention controls as the active data.</li>
        </ul>
      </section>

      <section id="security">
        <h2>9. Security</h2>
        <p>DEETNUTS uses safeguards appropriate to the nature of the Service, including HTTPS in production, an origin restricted to authenticated Cloudflare traffic, exact-email and multi-factor controls for the administrator API, origin-side validation of Access token signatures and audiences, secure response headers, non-root isolated containers, restricted server credentials, input validation, request-size and rate limits, no-store responses for personalised or administrative traffic, authenticated account checks, provider access controls, checksum validation for cutoff artifacts and separation of public pages from account features.</p>
