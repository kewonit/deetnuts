...
export default function OpenSourceNotices() {
  ...
  return (
    <PolicyPage title="Open-source notices" summary="Software credits and notices are kept concise, traceable and separate from the public admissions-data methodology.">
      ...
      <section><h2>Source for this version</h2>{sourceCommit ? <p>The deployed source corresponds to <a href={`https://github.com/kewonit/deetnuts/tree/${sourceCommit}`}>commit {sourceCommit}</a>.</p> : <p>This local build has no deployment commit identifier. Production deployments expose the corresponding commit when the hosting environment supplies it.</p>}</section>
