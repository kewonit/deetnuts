...
export const getCollegeDetails = cache(async (collegeId: string) => {
    ...
    const collegeRecord = await pb.collection('2024_mht_cet_colleges').getFirstListItem(`college_id="${collegeId}"`);
    ...
    const seatMatrixRecords = await pb.collection('2024_seat_matrix').getFullList({
        filter: `college_code="${paddedCollegeCode}"`,
    });
    const seatMatrix = seatMatrixRecords as unknown as SeatMatrixData[];

    // 4. Fetch cutoff data
    const cutoffRecords = await pb.collection('2024_mht_cet_round_one_cutoffs_duplicate').getFullList({
        filter: `college_code="${paddedCollegeCode}"`,
    });
...
}, ['collegeDetails'], { revalidate: 60 * 60 * 24 }); // Cache for 24 hours
