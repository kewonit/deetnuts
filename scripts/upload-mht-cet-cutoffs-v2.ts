...
class MHTCETCutoffUploader {
  ...
  async authenticateWithCredentials() {
    ...
    for (const authMethod of authMethods) {
      try {
        ...
        console.log(
          `❌ ${authMethod.name} authentication failed:`,
      ...
      const uploadPromises = batch.map((record, j) => {
        ...
        return this.pb
          ...
          .catch((error: any) => {
            errorCount++;
            console.error(
              `❌ Error uploading record ${recordIndex + 1}:`,
              error.response?.message || error.message,
            );
    ...
    try {
      ...
      try {
        ...
        console.error(
          ...
          listError.response?.message || listError.message,
        );
      }
  ...
  async run() {
    try {
      ...
      if (!canAccess) {
        console.error(
          `❌ Cannot access collection '${this.collectionName}'. Please ensure:`,
        );
