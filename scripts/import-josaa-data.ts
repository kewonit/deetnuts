...
async function main() {
  ...
  if (!ADMIN_EMAIL || !ADMIN_PASSWORD) {
    console.error(
      "ERROR: POCKETBASE_ADMIN_EMAIL and POCKETBASE_ADMIN_PASSWORD not set",
    );
  ...
  try {
    ...
    for (const inst of institutes) {
      try {
        ...
        if (institutesErrors <= 3) {
          console.error(
            `  Error creating ${inst.short_code}: ${error?.message || error}`,
          );
        }
      }
    }
    ...
    for (const branch of branches) {
      try {
        ...
        if (branchesErrors <= 3) {
          console.error(
            `  Error creating ${branch.short_code}: ${error?.message || error}`,
          );
        }
      }
    }
    ...
    const answer = await new Promise<string>((resolve) => {
      rl.question(
        `Import ${cutoffs.length.toLocaleString()} cutoffs? This may take a while (y/n): `,
        resolve,
      );
    });
    rl.close();
    ...
    for (let i = 0; i < cutoffs.length; i += BATCH_SIZE) {
      ...
      if (progress % 1000 === 0 || progress === cutoffs.length) {
        ...
        console.log(
          `  Progress: ${progress.toLocaleString()} / ${cutoffs.length.toLocaleString()} (${percent}%) - ${rate} records/sec`,
        );
      }
    ...
    console.log("=".repeat(60));
    console.log(
      `Institutes: ${institutesCreated} created, ${institutesSkipped} skipped`,
    );
    console.log(
      `Branches:   ${branchesCreated} created, ${branchesSkipped} skipped`,
    );
    console.log(
      `Cutoffs:    ${cutoffsCreated.toLocaleString()} created, ${cutoffsErrors.toLocaleString()} errors`,
    );
    console.log("");
    ...
    if (error?.status === 400) {
      console.error(
        "This may be an authentication error. Check your admin credentials.",
      );
    }
    process.exit(1);
  }
