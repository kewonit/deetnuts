...
class BatchSeatMatrixUploader {
        ...
        .on("data", (row) => {
          ...
          try {
            ...
            const record: SeatMatrixRecord = {
              ...
              PWD_common_reserved: this.parseIntegerField(
                row.PWD_common_reserved,
              ),
              DEF_total: this.parseIntegerField(row.DEF_total),
              DEF_common_reserved: this.parseIntegerField(
  ...
  async createBatch(
    ...
    return await batch.send({
      requestKey: `batch_create_seat_matrix_${batchId}`,
    });
  }
  ...
  async upsertBatch(
    ...
    // Add unique request key to prevent auto-cancellation
    return await batch.send({
      requestKey: `batch_upsert_seat_matrix_${batchId}`,
    });
  }

  async processBatchesConcurrently(
    batches: SeatMatrixRecord[][],
  ...
  async uploadData(operation: "create" | "upsert" = "create"): Promise<void> {
    try {
      ...
      const invalidRecords = records.filter(
        (r) => !r.college_code || !r.choice_code,
      );
  ...
  async clearCollection(): Promise<void> {
    try {
      ...
      while (hasMore) {
        ...
        await batch.send({
          requestKey: `batch_delete_seat_matrix_${page}_${Date.now()}`,
        });
  ...
  async validateCSV(): Promise<void> {
    try {
      ...
      for (let i = 0; i < sampleSize; i++) {
        ...
        console.log(
          `${i + 1}. College: ${record.college_code} - ${record.college_name}`,
        );
        console.log(`   Course: ${record.choice_code} - ${record.course_name}`);
        console.log(
          `   Total Seats: ${record.Total}, CAP Seats: ${record.CAP_seats}`,
        );
      ...
      // Show statistics
      const collegeCounts = records.reduce(
        (acc, record) => {
      ...
      const seatTypeCounts = records.reduce(
        (acc, record) => {
      ...
      topColleges.forEach(([collegeCode, count]) => {
        const collegeName =
          records.find((r) => r.college_code === collegeCode)?.college_name ||
          "Unknown";
  ...
  async listRecords(limit: number = 10): Promise<void> {
    try {
      ...
      result.items.forEach((record, index) => {
        console.log(
          `${index + 1}. ${record.college_code} - ${record.college_name}`,
        );
        console.log(`   Course: ${record.choice_code} - ${record.course_name}`);
        console.log(
          `   Total Seats: ${record.Total}, CAP Seats: ${record.CAP_seats}`,
  ...
  async getStatistics(): Promise<void> {
    try {
      ...
      const totalCAPSeats = records.reduce(
      ...
      const seatTypeStats = records.reduce(
        (acc, record) => {
          acc[record.seat_type] = (acc[record.seat_type] || 0) + 1;
          return acc;
        },
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log(
        "  npm run batch-upload-seat-matrix upsert    - Upsert seat matrix records (create or update)",
      );
      console.log(
        "  npm run batch-upload-seat-matrix clear     - Clear all seat matrix records",
      );
      console.log(
        "  npm run batch-upload-seat-matrix replace   - Clear and create (full replace)",
      );
      console.log(
        "  npm run batch-upload-seat-matrix list [n]  - List first n seat matrix records (default: 10)",
      );
      console.log(
        "  npm run batch-upload-seat-matrix validate  - Validate CSV file without uploading",
      );
      console.log(
        "  npm run batch-upload-seat-matrix stats     - Show collection statistics",
      );
      console.log("");
