...
class BatchCollegeUploader {
                ...
                .on('data', (row) => {
                    ...
                    try {
                        ...
                        if (!row.college_id || !row.college_name) {
                            console.warn(`⚠️  Skipping line ${lineNumber}: Missing college_id or college_name`);
                            skippedRecords++;
                            return;
                        }
                        ...
                        if (record.college_id === 0) {
                            console.warn(`⚠️  Skipping line ${lineNumber}: Invalid college_id`);
                            skippedRecords++;
                            return;
                        }
    ...
    async processBatchesConcurrently(batches: CollegeRecord[][], operation: 'create' | 'upsert' = 'create'): Promise<void> {
    ...
    async uploadData(operation: 'create' | 'upsert' = 'create'): Promise<void> {
        try {
            ...
            // Validate data
            const invalidRecords = records.filter(r => !r.college_id || !r.college_name);
            if (invalidRecords.length > 0) {
                console.warn(`⚠️  Found ${invalidRecords.length} records with missing college_id or college_name`);
            }
            ...
            console.log(`🎉 Successfully ${operation === 'upsert' ? 'upserted' : 'created'} ${records.length} college records!`);
    ...
    async clearCollection(): Promise<void> {
        try {
            ...
            while (hasMore) {
                ...
                await batch.send({ requestKey: `batch_delete_colleges_${page}_${Date.now()}` });
                totalDeleted += result.items.length;
                console.log(`🗑️  Deleted ${result.items.length} college records (${totalDeleted} total)`);
    ...
    async validateCSV(): Promise<void> {
        try {
            ...
            for (let i = 0; i < sampleSize; i++) {
                ...
                console.log(`${i + 1}. ID: ${record.college_id}, Name: ${record.college_name}`);
            ...
            const missingUniversity = records.filter(r => !r.home_university).length;
            if (missingUniversity > 0) {
                console.log(`⚠️  ${missingUniversity} records have missing home_university`);
            }
...
async function main() {
    ...
    switch (command) {
        ...
        default:
            console.log('📋 Usage:');
            console.log('  npm run batch-upload-colleges create    - Create new college records');
            console.log('  npm run batch-upload-colleges upsert    - Upsert college records (create or update)');
            console.log('  npm run batch-upload-colleges clear     - Clear all college records');
            console.log('  npm run batch-upload-colleges replace   - Clear and create (full replace)');
            console.log('  npm run batch-upload-colleges list [n]  - List first n college records (default: 10)');
            console.log('  npm run batch-upload-colleges validate  - Validate CSV file without uploading');
            console.log('');
