...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={cn(
      "fixed inset-0 z-50 bg-overlay font-bold data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 motion-reduce:animate-none",
      className,
    )}
    {...props}
    ref={ref}
...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-white dark:bg-darkBg text-text dark:text-darkText p-6 transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500 motion-reduce:animate-none motion-reduce:transition-none",
  {
    variants: {
      side: {
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      <SheetPrimitive.Close className="absolute right-3 top-3 z-10 inline-flex min-h-11 min-w-11 items-center justify-center border border-slate-300 bg-white ring-offset-white hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-violet-700 focus:ring-offset-2 disabled:pointer-events-none">
        <X className="h-4 w-4" />
