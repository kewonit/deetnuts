...
export async function CollegeYearPage({ exam, slug, year }: { exam: JeeExamId; slug: string; year: number }) {
  ...
  return <main className="cutoff-main"><JsonLd value={jsonLd} /><Breadcrumbs items={[{ label: "JEE cutoffs", href: "/jee-cutoffs" }, { label: `${label} colleges`, href: `/${exam}/colleges` }, { label: college.seoName, href: cutoffPath(exam, slug) }, { label: String(year) }]} />
    ...
    <section className="cutoff-source-card"><div><h2>Source records</h2><p>Publisher records used for this college and year.</p></div><ul className="cutoff-source-list">{sources.map((source) => <li key={source.sourceId}><strong>{source.title}</strong><span>{source.officialDomain}</span></li>)}</ul><p className="cutoff-disclaimer" data-nosnippet>DEETNUTS is independent of JoSAA, CSAB, NTA and the colleges listed. Always confirm admission decisions on the applicable official portal. Source access does not grant reuse rights; see <Link href="/compliance/data-sources-and-licensing">data &amp; licensing</Link>.</p></section>
  </main>;
}
